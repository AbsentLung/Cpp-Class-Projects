...
stack::stack(int cap) { //Constructor declared with the capacity
	...
	_arr = new string[_capacity]; //The array is equal to a new integer array the size of the capacity
...
stack::stack(const stack& stk) { //Copy constructor declared with the stack
	if ( stk._arr == 0 ) { //If empty
		...
		_arr = new string[_capacity]; //New array with the same size
...
void stack::push(const string input) { //Pushes an element of value "x" into the stack
	_arr[_tos] = input; //Assign element to the top of the stack
	...
	if (_capacity == _tos) { //if the capacity is equal to the top of the stack
		...
		string temp [_capacity]; //An temp version of the array is created
		for (int i = 0; i < _tos; i++)
			temp[i] = _arr[i];
		delete [] _arr;
		_arr = new string [_capacity]; //The array is recreated with the new size
...
string& stack::top() { //Returns the value at the top of the stack
...
stack& stack::operator=(const stack& stk) { //Make one stack equal to another stack
	if (this != &stk) { //If they aren't already equal
		if (stk._arr == 0) { //If array is empty
			...
			_tos = stk._tos; //Top of stack is the same
			delete [] _arr; //Delete array
			_arr = new string[_capacity]; //Array has new capacity
...
void stack::print(std::ostream& os){ // print stack content to ostream
	os << "Contents: ";
	for(int i = 0; i<_tos; i++) //For every character in the stack
		os<<_arr[i]<<' '; //Character in the stack gets print out followed by a space.
	os << endl;
}
